...
class TestScrubControls(CephFSTestCase):
    ...
    def _abort_scrub(self, expected):
        ...
        res = json.loads(self.fs.mon_manager.raw_cluster_cmd("tell", "mds.{0}".format(mds_id), "scrub", "abort"))
        self.assertEqual(res['return_code'], expected)
    def _pause_scrub(self, expected):
        mds_id = self.fs.get_lone_mds_id()
        res = json.loads(self.fs.mon_manager.raw_cluster_cmd("tell", "mds.{0}".format(mds_id), "scrub", "pause"))
        self.assertEqual(res['return_code'], expected)
    def _resume_scrub(self, expected):
        mds_id = self.fs.get_lone_mds_id()
        res = json.loads(self.fs.mon_manager.raw_cluster_cmd("tell", "mds.{0}".format(mds_id), "scrub", "resume"))
        self.assertEqual(res['return_code'], expected)
    def _scrub_status(self):
        mds_id = self.fs.get_lone_mds_id()
        return json.loads(self.fs.mon_manager.raw_cluster_cmd("tell", "mds.{0}".format(mds_id), "scrub", "status"))
    ...
    def test_scrub_abort(self):
        ...
        mds_id = self.fs.get_lone_mds_id()
        out_json = json.loads(self.fs.mon_manager.raw_cluster_cmd(
            "tell", "mds.{0}".format(mds_id), "scrub_path", abs_test_path, "recursive"))
        self.assertNotEqual(out_json, None)

        # abort and verify
        self._abort_scrub(0)
        out_json = self._scrub_status()
