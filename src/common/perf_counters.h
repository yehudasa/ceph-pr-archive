...
class perf_counters_t : public PerfCountersCollectionable {
  ...
  void _inc(const std::size_t amount = 1) {
    ...
    // Yes, tons of templated code are here also to put on hot paths as
    // few instructions as possible:
    //
    //    data16 data16 data16 mov %fs:0x0,%rax
    //    mov    -0x30d0(%rax),%rdx
    //    cmp    $0x1f,%rdx
    ...
    // and third ones in ::log_op_stats costs single instruction:
    //
    //    add    %r15,0x90(%rdx)
    //    add    %r14,0x98(%rdx)
