...
//	if (hash_uri(oid).compare(HASH_LOCAL) != 0) {
//		dout(0) << "Engage1: Warning: We are only L2, no write:" << oid << dendl;
//		return;
//	}

	ldout(cct, 0) << "Engage1: We are in DataCache::put() and oid is: " << oid <<dendl;
	cache_lock.Lock();
	map<string, ChunkDataInfo *>::iterator iter = cache_map.find(oid);
	if (iter != cache_map.end()) {
		cache_lock.Unlock();
		ldout(cct, 0) << "Engage1: Warning: data already cached, no rewirte" << dendl;
		return;
	}
	...
	if (it != outstanding_write_list.end()) {
		cache_lock.Unlock();
		ldout(cct, 0) << "Engage1: Warning: data put in cache already issued, no rewrite" << dendl;
		return;
	}
	...
	ldout(cct, 0) << "Engage1: Before eviction _free_data_cache_size:" << _free_data_cache_size << ", _outstanding_write_size:" << _outstanding_write_size << "freed_size:" << freed_size << dendl;
	while (len >= (_free_data_cache_size - _outstanding_write_size + freed_size)){
		ldout(cct, 0) << "Engage1: enter eviction, r=" << r << dendl;
		r = lru_eviction();
		//r = random_eviction();
		if(r < 0)
			return;
	...
	if (r < 0) {
		...
		cache_lock.Unlock();
		ldout(cct, 0) << "Engage1: create_aio_wirte_request fail, r=" << r << dendl;
		return;
	}
...
size_t DataCache::random_eviction(){
	...
	ldout(cct, 0) << "INFO::random_eviction index:"<< random_index << ", free size:0x" << std::hex << del_entry->size << dendl;
...
size_t DataCache::lru_eviction(){
	...
	ldout(cct, 0) << "Engage1: lru_eviction: oid to remove" << del_oid << dendl;
...
void DataCache::remote_io(struct librados::L2CacheRequest *l2request ) {
	ldout(cct, 0) << "Engage1: Add task to remote IO" << dendl;
...
std::string DataCache::hash_uri(std::string oid) {
	...
	return "192.168.67.9:80"; // HASH_LOCAL;
}
...
string DataCache::deterministic_hash(string oid) {
  std::string::size_type sz;   // alias of size_t
  ...
  {
    case 0:
      return "172.20.0.7:80";
    case 1:
      return "172.20.0.8:80";
    case 2:
      return "172.20.0.9:80";
    case 3:
      return "172.20.0.10:80";
  };
  return "";
}
...
int HttpL2Request::submit_http_request () {
  ...
  std::string range = std::to_string(req->ofs + req->read_ofs)+ "-"+ std::to_string(req->ofs + req->read_ofs + req->len - 1);
  struct curl_slist *header = NULL;
  get_obj_data *d = (get_obj_data *)req->op_data;
  struct req_state *s;
  ((RGWGetObj_CB *)(d->client_cb))->get_req_info(s);
  ...
  if (s->dialect == "s3") {
    ...
    if (!s->http_auth || !(*s->http_auth)) {
      ...
      std::string auth_str(s->http_auth + 4);
    ...
    if (iter == s->user->access_keys.end()) {
      dout(cct, 10) << "ERROR: access key not encoded in user info" << dendl;
      return -EPERM;
    }
  ...
  if(res != CURLE_OK)
  {
//	  dout(cct, 0) << "Engage1: curl_easy_perform() failed " << curl_easy_strerror(res) << " oid " << req->oid << " offset " << req->ofs + req->read_ofs  <<dendl;
	  return -1;
  }
  return 0;
}
...
int HttpL2Request::sign_request(RGWAccessKey& key, RGWEnv& env, req_info& info)
{
  ...
  if (cct->_conf->subsys.should_gather(ceph_subsys_rgw, 20)) {
    for (const auto& i: env.get_map()) {
      //dout(20) << "> " << i.first << " -> " << rgw::crypt_sanitize::x_meta_map{i.first, i.second} << dendl;
  ...
  if (!rgw_create_s3_canonical_header(info, NULL, canonical_header, false)) {
    //dout(0) << "failed to create canonical s3 header" << dendl;
    return -EINVAL;
  }

  //dout(cct, 15) << "generated canonical header: " << canonical_header << dendl;
  ...
  std::string auth_hdr = "AWS " + key.id + ":" + digest;
  dout(cct, 15) << "generated auth header: " << auth_hdr << dendl;
