...
TEST(LibCephFS, RstatsPropagate) {
  struct ceph_mount_info *cmount;
  ASSERT_EQ(0, ceph_create(&cmount, NULL));
  ASSERT_EQ(0, ceph_conf_read_file(cmount, NULL));
  ASSERT_EQ(0, ceph_conf_parse_env(cmount, NULL));
  ...
  char val[1024];
  int a = 1;
  ASSERT_EQ(0, ceph_setxattr(cmount, dir_path, "ceph.rstats.propagate", &a, sizeof(int), 0));
  ASSERT_LE(0, ceph_getxattr(cmount, dir_path, "ceph.dir.rctime", &val[0], 1024));
  ASSERT_LE(0, ceph_statx(cmount, c_path, &stx, CEPH_STATX_ALL_STATS, 0));

  std::stringstream ss;
  ss << stx.stx_ctime.tv_sec << "." << stx.stx_ctime.tv_nsec;
  std::cout<< "dir_rctime: " << val << ", f_mtime: " << ss.str() << std::endl;
  ASSERT_EQ(std::string(val), ss.str());
