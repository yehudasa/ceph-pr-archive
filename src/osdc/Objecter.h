...
struct ObjectOperation {
  ...
  void add_xattr(int op, std::string_view name, const bufferlist& data) {
    OSDOp& osd_op = add_op(op);
  ...
  struct CB_ObjectOperation_stat {
    ...
    void operator()(boost::system::error_code ec, int r, const bufferlist& bl) {
      if (r >= 0) {
	auto p = bl.cbegin();
	try {
	  uint64_t size;
  ...
  struct CB_ObjectOperation_cmpext {
    ...
    void operator()(boost::system::error_code ec, int r, const bufferlist&) {
      if (prval)
        *prval = r;
      if (this->ec)
	*this->ec = ec;
  ...
  void cmpext(uint64_t off, bufferlist&& cmp_bl, boost::system::error_code* ec,
	      std::size_t* s) {
    add_data(CEPH_OSD_OP_CMPEXT, off, cmp_bl.length(), cmp_bl);
  ...
  struct CB_ObjectOperation_sparse_read {
    ...
    int* prval;
    boost::system::error_code* pec;
    CB_ObjectOperation_sparse_read(bufferlist* data_bl,
				   Ex* extents,
				   int* prval,
				   boost::system::error_code* pec)
      : data_bl(data_bl), extents(extents), prval(prval), pec(pec) {}
    void operator()(boost::system::error_code ec, int r, const buffer::list& bl) {
      auto iter = bl.cbegin();
      if (r >= 0) {
  ...
  void getxattrs(boost::system::error_code* ec,
		 boost::container::flat_map<std::string, bufferlist> *pattrs) {
    add_op(CEPH_OSD_OP_GETXATTRS);
  ...
  void setxattr(std::string_view name, const bufferlist& bl) {
    add_xattr(CEPH_OSD_OP_SETXATTR, name, bl);
  }
  ...
  void omap_get_vals(std::optional<std::string_view> start_after,
		     ...
		     uint64_t max_to_get,
		     boost::system::error_code* ec,
		     boost::container::flat_map<std::string, bufferlist> *out_set,
		     bool *ptruncated) {
    OSDOp &op = add_op(CEPH_OSD_OP_OMAPGETVALS);
    bufferlist bl;
    encode(start_after ? *start_after : string_view{}, bl);
    encode(max_to_get, bl);
    encode(filter_prefix ? *start_after : string_view{}, bl);
    op.op.extent.offset = 0;
    op.op.extent.length = bl.length();
    op.indata.claim_append(bl);
    set_handler(CB_ObjectOperation_decodevals(max_to_get, out_set, ptruncated,
  ...
  void omap_cmp(const boost::container::flat_map<
		  std::string, pair<bufferlist, int>>& assertions,
		boost::system::error_code *ec) {
    OSDOp &op = add_op(CEPH_OSD_OP_OMAP_CMP);
    bufferlist bl;
    encode(assertions, bl);
  ...
  void omap_get_header(boost::system::error_code* ec, bufferlist *bl) {
    add_op(CEPH_OSD_OP_OMAPGETHEADER);
    out_bl.back() = bl;
    out_ec.back() = ec;
  }
  ...
  void omap_set(const boost::container::flat_map<string, bufferlist>& map) {
    bufferlist bl;
    encode(map, bl);
    add_data(CEPH_OSD_OP_OMAPSETVALS, 0, bl.length(), bl);
  }
  ...
  void call(std::string_view cname, std::string_view method,
	    const bufferlist& indata, boost::system::error_code* ec) {
    add_call(CEPH_OSD_OP_CALL, cname, method, indata, NULL, NULL, NULL);
    out_ec.back() = ec;
  }

  void call(std::string_view cname, std::string_view method, const bufferlist& indata,
	    boost::system::error_code* ec, bufferlist *outdata) {
...
public:
  ...
  static cf::unique_function<void(boost::system::error_code)> OpContextVert(Context* c) {
  ...
  struct Op : public RefCountedObject {
    ...
    Op(const object_t& o, const object_locator_t& ol, vector<OSDOp>&& _ops,
       int f, cf::unique_function<void(boost::system::error_code)> fin,
    ...
    Op(const object_t& o, const object_locator_t& ol, vector<OSDOp>&& _ops,
       int f, Context* fin, version_t *ov,
       int *offset = NULL, ZTracer::Trace *parent_trace = nullptr) :
      target(o, ol, f),
      ops(std::move(_ops)),
      out_bl(ops.size(), nullptr),
      out_handler(ops.size()),
...
public:
  ...
  Op *prepare_mutate_op(
    ...
    ceph_assert(o->ops.size() == o->out_bl.size());
    ceph_assert(o->ops.size() == o->out_rval.size());
    ceph_assert(o->ops.size() == o->out_ec.size());
    ceph_assert(o->ops.size() == o->out_handler.size());
  ...
  void mutate(const object_t& oid, const object_locator_t& oloc,
	      ObjectOperation&& op, const SnapContext& snapc,
	      ceph::real_time mtime, int flags,
	      cf::unique_function<void(boost::system::error_code)> oncommit,
  ...
  void read(const object_t& oid, const object_locator_t& oloc,
	    ObjectOperation&& op, snapid_t snapid, bufferlist *pbl, int flags,
	    cf::unique_function<void(boost::system::error_code)> onack,
