...
class ZFSPath(object):
    ...
    def __call__(self, string):
        ...
        try:
            ...
            error = "Logical volume must be specified as 'zpool/zfsvol' but got: %s" % string
            raise argparse.ArgumentError(None, error)

        if not zpool:
            error = "Didn't specify a zpool like 'zpool/zfsvol', got: %s" % string
        if not zfs:
            error = "Didn't specify a zfs volume like 'zpool/zfsvol', got: %s" % string
