...
that prefixes tags with ``ceph:`` and uses 
...
def is_zfs(dev, zfsfs=None):
    ...
    if splitname.get('ZFS_NAME'):
        zfsfs.filter(name=splitname['ZFS_NAME'], zpool_name=splitname['POOL_NAME'])
        return len(zfsfs) > 0
    return False
...
def get_zfs(name=None, zpool_name=None, mountpoint=None, guid=None, zfs_tags=None):
    """
    Return a matching zfs for the current system, requiring ``name``,
    ``zpool_name``, ``mountpoint`` or ``tags``. Raises an error if more than one lv
    is found.
...
def get_zfs(name=None, zpool_name=None, mountpoint=None, guid=None, zfs_tags=None):
    """
    Return a matching zfs for the current system, requiring ``name``,
    ``zpool_name``, ``mountpoint`` or ``tags``. Raises an error if more than one 
    zfs is found.
...
def get_zpool(zpool_name=None):
    """
    Return a matching zpool (physical volume) for the current system, requiring
    ``zpool_name``. Raises an error if more than one
    zpool is found.
    """
...
def create_zpool(device, osd_id):
    ...
    process.run(['gpart','add','-a', '1M', '-t', 'freebsd-zfs', 
                 '-l', osd, device ],
        )
    # Then create the ZFS filesystem on the partion using the label
    process.run(['zpool', 'create',osd, 
                 '/dev/gpt/%s' % osd ],
        )
    zpools = ZPools()
    return zpools.get(name=osd)
...
def create_zfsvol(device, osd_id, tags=None):
    ...
    process.run([
        'zfs', 'create',
        '-o', 'mountpoint=/var/lib/ceph/osd/%s' % osd,
	'-o', 'compression=on',
        '-o', 'checksum=off',
        '-o', 'atime=off',
        '-o', 'devices=off',
...
def create_zfs(zpool, parts=None, size=None, name_prefix='ceph-zfs'):
    ...
    .. note:: ZVs that are not in use can be detected by querying ZFS for tags that are
              set to ``"null"``.
    ...
    :type parts: int
    :param size: Size (in gigabytes) of ZVs to create, e.g. "as many 10gb ZVs as possible"
    :type size: int
    """
...
class ZPools(list):
	...
	fields = 'name,guid,size,comment'
        ...
        It is useful to use ``tags`` when trying to find a specific logical volume,
        but it can also lead to multiple zfs being found, since a lot of metadata
        is shared between zfs of a distinct OSD.
        """
...
class ZVolumes(list):
    ...
    def _filter(self, name=None, zpool_name=None, mountpoint=None, guid=None, zfs_tags=None):
        """
        The actual method that filters using a new list. Useful so that other
        methods that do not want to alter the contents of the list (e.g.
        ``self.find``) can operate safely.
    ...
    def filter(self, name=None, zpool_name=None, mountpoint=None, 
        ...
        """
        if not any([name, zpool_name, mountpoint, guid, zfs_tags]):
            raise TypeError('.filter() requires name, pool_name, mountpoint, guid, or tags (none given)')
    ...
    def get(self, name=None, zpool_name=None, mountpoint=None, guid=None, zfs_tags=None):
        """
        This is a bit expensive, since it will try to filter out all the
        matching items in the list, filter them out applying anything that was
        added and return the matching item.
...
class ZVolume(object):
    ...
    def set_tags(self, newtags):
        ...
        # zfs_* identifiers to filter because those shouldn't change
