...
class MGRDpPool(BaseDP):
    _fields = [
        ...
        'pg_num', 'pgp_num', 'created_time', 'pgids', 'osd_ids'
    ]
...
class MGRDpDisk(BaseDP):
    _fields = ['host_domain_id', 'host', 'osd_id', 'osd_type']
...
class DBRelayAgent(MetricsAgent):
        ...
        for osdid, o_val in self._osd_nodes.items():
            ...
            for dev in o_devs:
                ...
                if disk_domain_id not in self._dev_nodes.keys():
                    ...
                    if osdid not in dev_node.meta.get('osd_id', '').split(','):
                        dev_node.meta['osd_id'] += ',' + str(osdid)
                    for t in d_type:
                        if t not in dev_node.meta.get('osd_type', '').split(','):
                            dev_node.meta['osd_type'] += (',' + t)
    ...
    def _osd_contains_disk(self):
        ...
        for d_name, d_node in self._dev_nodes.items():
            for osdid in d_node.meta.get('osd_id', '').split(','):
                data = DBRelay()
